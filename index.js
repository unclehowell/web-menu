...
startupSound.play().catch(() => {
    console.warn("Startup sound autoplay was prevented by the browser.");
});
...
let sideClock = () => {
    ...
    let clock = `${d.getDate()}/${d.getMonth()+1} ${d.getHours()}:${minutes}`
...
let focusSubMenu = (sn, sub, down, up) =>{
    switch(sub){
        case 0:
            if(up){
                if(submenu[sub+1][sn]) submenu[sub+1][sn].classList.remove("active");
                if(submenu[sub][sn]) submenu[sub][sn].classList.remove("inactive");
            }
            break;
        case 1:
            if(down){
                if(submenu[sub-1][sn]) submenu[sub-1][sn].classList.add("inactive");
                if(submenu[sub][sn]) submenu[sub][sn].classList.add("active");
            }
            else if(up){
                if(multiSection){
                    if(submenu[sub+1][sn-1]) submenu[sub+1][sn-1].classList.remove("active");
                    if(submenu[sub-1][sn]) submenu[sub-1][sn].classList.remove("gotop");
                    if(submenu[sub][sn]) submenu[sub][sn].classList.add("active");
                } else {
                    if(submenu[sub+1][sn]) submenu[sub+1][sn].classList.remove("active");
                    if(submenu[sub][sn]) submenu[sub][sn].classList.add("active");
        ...
        case 2:
            if(down){
                if (multiSection) {
                    if(submenu[sub-2][sn]) submenu[sub-2][sn].classList.add("gotop");
                    if(submenu[sub-1][sn]) submenu[sub-1][sn].classList.remove("active");
                    if(submenu[sub][sn - 1]) submenu[sub][sn - 1].classList.add("active");
                } else {
                    if(submenu[sub-1][sn]) {
